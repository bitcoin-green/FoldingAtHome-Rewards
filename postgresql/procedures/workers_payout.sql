...
WHERE
    ...
    (
        SELECT DISTINCT
            ON (recent.lastupdate) recent.lastupdate
        FROM
            fath_workers recent
        ORDER BY
            recent.lastupdate DESC LIMIT 1
    )
ORDER BY
    worker.name DESC;
...
WHERE
    ...
    (
        SELECT DISTINCT
            ON (previous.lastupdate) previous.lastupdate
        FROM
            fath_workers previous
        ORDER BY
            previous.lastupdate DESC OFFSET 1 LIMIT 1
    )
ORDER BY
    worker.name DESC;
...
FROM
    tmp_recent_snapshot R
    INNER JOIN
        tmp_previous_snapshot P
        ON R.folding_id = P.folding_id;
